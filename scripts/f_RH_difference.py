...
def plot_absolute(f_RH_main, f_RH, RH, savedir, aer_order, band_order, lam_colours, Q_type):
    ...
    for ax_i, aerosol in zip(ax.flatten(), aer_plot_order):
        ...
        #
        # # plot 910 nm
        # ax_i.plot(RH, f_RH_main['910-910nm'][aerosol], color='black', linestyle='--', label='910-910nm')
    ...
    plt.savefig(savedir + 'absolute/multi-panel-band_absolute_vs910'+ '_' + Q_type[0:3] + '_f_RH.png')
    plt.close()

    return fig


def plot_difference(f_RH_diff, RH, savedir, aer_order, band_order, lam_colours, Q_type):
    ...
    plt.savefig(savedir + 'difference/multi-panel-band_difference_vs910'+ '_' + Q_type[0:3] + '_f_RH.png')
    plt.close()

    return fig


def plot_ratio(f_RH_ratio, RH, savedir, aer_order, band_order, lam_colours, Q_type):
    ...
    for ax_i, aerosol in zip(ax.flatten(), aer_plot_order):
        ...
        for lam_range, lam_i_colour in zip(band_order, lam_colours):
            ...
            ax_i.plot(RH, f_RH_ratio[lam_range][aerosol], color=lam_i_colour, label=lam_range)

        # prettify subplot
        ax_i.set_title(aerosol)
        #ax_i.set_ylim([1.05, 1.3])
        ax_i.set_xlim([0.0, 100.0])
    ...
    plt.savefig(savedir + 'ratio/multi-panel-band_ratio_vs910'+ '_' + Q_type[0:3] + '_f_RH.png')
    plt.close()
...
def main():
    ...
    file_names = ['sp_908-912_r1.1e-7_stdev1.05_num1.0e8',
                  'sp_908-912_r1.1e-7_stdev1.05_num8.0e9',
                  'sp_908-912_r1.1e-7_stdev1.05_num2.0e10']
    ...
    fig = plot_absolute(f_RH_main, f_RH, RH, savedir, aer_order, lam_colours, Q_type)
